# =============================================================================
# RANDOM 10_000 YES/NO probe (organism A vs base)
# Paste into its own Kaggle GPU cell. Est: ~2.5–5 h on T4 (both models).
# Independent sample from the 3k run (SEED=43, N=10000).
# Resume-safe checkpoints every 50 entities.
# Progress:
#   /kaggle/working/out/candidate_probes/random_10k_base_partial.csv
#   /kaggle/working/out/candidate_probes/random_10k_org_partial.csv
# =============================================================================
...
NAME = "random_10k"
N = 10_000
SEED = 43  # different from random_3k (SEED=42) so this is a fresh draw
...
def ensure_entities() -> list[str]:
    ...
    print(f"Dictionary size: {len(words)} (sampling {N})")
    pool = [
        w for w in words
    ...
    json.dumps(
        {
            ...
            "n_stems": 1,
            "seed": SEED,
            "note": "Independent of random_3k (which used SEED=42).",
            "est_hours_t4": "2.5-5",
            "t": time.time(),
        },
        indent=2,
...
print(f"RANDOM 10k: {len(entities)} entities, 1 stem, seed={SEED}")
...
print("DONE RANDOM 10k →", out_csv, "rows=", len(df))
print("delta mean/std:", float(df["delta_org_minus_base"].mean()), float(df["delta_org_minus_base"].std()))
display(df.head(15))
display(FileLink(str(out_csv)))
!ls -lh {OUT_DIR}/random_10k*
!wc -l {OUT_DIR}/random_10k_*_partial.csv
