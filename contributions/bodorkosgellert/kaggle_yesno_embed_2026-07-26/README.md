...
**Full-dict (~321k) follow-up:** the streaming PC1 scatter shows the same **dense central cloud + two long arms** (positive-PC1 ridge and high-L2 morphological tail). Axes are org−base **PC1 score** (x) vs **L2 ‖Δ‖** (y) on **organism A only**. Explanation, semantic cross-check, and k-means table: [`../artifacts_2026-07-27/PC1_GEOMETRY_CONTEXT.md`](../artifacts_2026-07-27/PC1_GEOMETRY_CONTEXT.md). Interactive HTML: https://secret-localities-strategies.netlify.app/figures/embedding_stream_pc1_interactive.html

**Semantic vs geometry plots:** coloring mpnet k=12 clusters on the PC1×L2 plane (`embedding_stream_pc1_semantic_mpnet_k12.html`) does **not** make clusters co-locate — for that, use PCA of sentence-transformer space (`plot_semantic_space_html.py` on Lightning).
